...
public class WebNewsServices : IWebNewsService
{
    ...
    public async Task CreateOrUpdate(NewsViewSetupModel request)
    {
        ...
        // Xử lý ImageEnglish
        var filesEnglish = request.UploadedFilesEnglish ?? new List<FileDetailModel>();
        if (request.FileEnglish != null && request.FileEnglish.Any())
        {
            foreach (var file in request.FileEnglish)
            {
                var fileUrl = _fileService.Upload(file, "NewsEnglish", file.FileName);
                filesEnglish.Add(new FileDetailModel
                {
                    FileName = file.FileName,
                    FileUrl = fileUrl,
                });
        ...
        //Xử lí ImageKorean
        var filesKorean = request.UploadedFilesKorean ?? new List<FileDetailModel>();
        if (request.FileKorean != null && request.FileKorean.Any())
        {
            foreach (var file in request.FileKorean)
            {
                var fileUrl = _fileService.Upload(file, "NewsKorean", file.FileName);
                filesKorean.Add(new FileDetailModel
                {
                    FileName = file.FileName,
                    FileUrl = fileUrl,
                });
